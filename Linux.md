...
- [Linux](#linux)
  ...
  - [Подключение внешнего HDD диска](#подключение-внешнего-hdd-диска)
...
## Подключение внешнего HDD диска
...
# Выключить вход root пользователя (убедиться что есть другой пользователь)
PermitRootLogin no
```
...
# Отчёт в машинно-читаемом формате
cat /var/log/lynis-report.dat
```

### needrestart

Нужен для проверки процессов и сервисов, которые продолжают использовать старые версии библиотек после обновления пакетов. Он также умеет определять необходимость перезагрузки после обновления ядра.

```bash
sudo apt install needrestart

# После обновления системы можно просто выполнить
sudo needrestart

# Только вывести список без перезапуска
sudo needrestart -r l
```

### debsums

Утилита для проверки целостности файлов, установленных из .deb пакетов.
...
# В современных Debian часто рекомендуют использовать (dpkg --verify)
# Работает быстрее, использует информацию самого dpkg, проверяет не только контрольные суммы, но и права доступа, владельца, размер и другие атрибуты файлов.
sudo dpkg -V
```
