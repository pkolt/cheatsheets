...
- [Linux](#linux)
  ...
  - [Сеть](#сеть)
    - [Работа с iproute2](#работа-с-iproute2)
    - [Ping](#ping)
...
Пришел на смену устаревшему пакету `net-tools` (содержащему команды `ifconfig`, `route`, `arp` и другие)

Состав пакета:

- `ip` - используется для показа и изменения параметров сети: маршрутизации, сетевых интерфейсов, туннелей и соседних устройств;
- `ss` - для получения детальной информации о сетевых соединениях (заменяет `netstat`);
- `tc` - для настройки контроля трафика (Traffic Control);
- `bridge` - для управления сетевыми мостами;
- `rtmon` - для мониторинга состояния маршрутизации;
